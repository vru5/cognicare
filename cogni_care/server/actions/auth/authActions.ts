...
export const registerUser = async (body: RegistrationBody) => {
    ...
    return await (prisma as any).$transaction(async (tx: any) => {
        ...
        if (role === "PATIENT") {
            ...
            return { message: "Registration successful", userId: user.id, profileId: patientProfile.id };
        } else if (role === "CARER") {
            ...
            if (patientId) {
                ...
                if (!patientExists) {
                    throw new Error(`Patient ID "${patientId}" not found. Please check the ID and try again.`);
                }
            ...
            return { message: "Registration successful", userId: user.id, profileId: carerProfile.id };
        }
...
export const getProfileAction = async (userId: string) => {
    try {
        ...
        if (patient) return { success: true, role: "PATIENT", profileId: patient.id };
...
export const loginUser = async (body: { email: string; password: string }) => {
    ...
    if (!user.password) {
        console.log("LOGIN ERROR: User exists but has no password (legacy account)");
        throw new Error("Invalid email or password");
    }
    ...
    // Resolve profile ID
    const patient = await (prisma as any).profilePatient.findUnique({ where: { userId: user.id } });
    if (patient) {
        return { success: true, userId: user.id, profileId: patient.id, role: "PATIENT", name: user.name };
    }

    const carer = await (prisma as any).profileCarer.findUnique({ where: { userId: user.id } });
    if (carer) {
        return { success: true, userId: user.id, profileId: carer.id, role: "CARER", name: user.name };
    }

    return { success: true, userId: user.id, profileId: null, role: user.role, name: user.name };
};
